...
struct RootView: View {
    ...
    private var backendLabel: String {
        if appState.isBackendOnline { return "Backend connected" }
        if backend.isRunning { return "Backend starting…" }
        if let err = backend.lastError { return err }
    ...
    private var backendRow: some View {
        HStack {
            Circle()
                .fill(appState.isBackendOnline ? Color.green : (backend.isRunning ? Color.orange : Color.red))
